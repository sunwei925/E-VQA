...
def extract_multiframe(videos_dir, video_name, save_folder, video_length_min):
    filename = os.path.join(videos_dir, video_name)
    video_name_str = video_name[:-4]
    ...
    frames_per_part = total_frames // video_length_min 

    extract_frames = []

    for part in range(video_length_min):
        start_fr
...
def main(config):
    ...
    elif config.dataset == 'NTIREVideoTrain':
        column_names = ['filename','score']
        dataInfo = pd.read_csv(config.dataset_file, header = 0, sep=',', names=column_names, index_col=False, encoding="utf-8-sig")
    ...
    elif config.dataset == 'NTIREVideoValidation':
    ...
    elif config.dataset == 'NTIREVideoTest':
        column_names = ['filename','score']
        dataInfo = pd.read_csv(config.dataset_file, header = 0, sep=',', names=column_names, index_col=False, encoding="utf-8-sig")
        video = dataInfo['filename'].tolist()
        n_video = len(video)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--resize', type=int, default=384) # 448 for ResNet-50, 384 for Swin-B
    parser.add_argument('--bid', type=int, default=0) # 448 for ResNet-50, 384 for Swin-B
    parser.add_argument('--eid', type=int, default=40000) # 448 for ResNet-50, 384 for Swin-B
