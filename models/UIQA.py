...
class Model_SwinT_LSVQ(nn.Module):
    def __init__(self):
        super(Model_SwinT_LSVQ, self).__init__()
        ...
        # quality regressor
        self.quality = self.quality_regression(768+256, 128, 1)
...
class UVQA_Model(torch.nn.Module):
    def __init__(self, pretrained_path=None):
        
        super(UVQA_Model, self).__init__()
        # Aesthetics feature extractor
        swin_t_aesthetics = Model_SwinT_LSVQ()
        if pretrained_path!=None:
            print('load aesthetics model')
            swin_t_aesthetics.load_state_dict(torch.load(pretrained_path))

        # Distortion feature extractor
        swin_t_distortion = Model_SwinT_LSVQ()
        if pretrained_path!=None:
            print('load distortion model')
            swin_t_distortion.load_state_dict(torch.load(pretrained_path))
        ...
        self.distortion_feature_extraction = swin_t_distortion.feature_extraction
        # self.saliency_feature_extraction = swin_t_salient.feature_extraction
        self.quality = self.quality_regression(768+768, 128, 1)
    ...
    def forward(self, x_aesthetics, x_distortion):

        a_size, d_size = x_aesthetics.shape, x_distortion.shape

        x_aesthetics = x_aesthetics.view(-1, a_size[2], a_size[3], a_size[4])
        x_distortion = x_distortion.view(-1, d_size[2], d_size[3], d_size[4])

        x_aesthetics = self.aesthetics_feature_extraction(x_aesthetics)
        x_distortion = self.distortion_feature_extraction(x_distortion)
        # x_saliency = self.saliency_feature_extraction(x_saliency)
        # fuse the features
        x = torch.cat((x_aesthetics, x_distortion), dim = 1)

        x = self.quality(x)
        x = x.view(a_size[0],a_size[1])
