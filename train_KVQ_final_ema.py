...
if __name__ == '__main__':
    ...
    if database == 'UHD_IQA':
        ...
        # filename_list = '/mnt/sda/fk/data/VQA/KVQ/trainval_data.csv'
        # database_dir = '/mnt/sda/fk/data/VQA/KVQ/trainval_original'
        filename_list = '/data/sunwei_data/NTIRE25_5W2_Videos/train_data.csv'
        database_dir = '/data/sunwei_data/NTIRE25_5W2_Videos/N_image_original'
        val_filename_list = '/data/sunwei_data/NTIRE25_5W2_Videos/val_data.csv'
        val_database_dir = '/data/sunwei_data/NTIRE25_5W2_Videos/N_image_original'
    ...
    if args.model == 'UIQA':
        ...
        model_ema = UIQA.UVQA_Model()
        model_ema.load_state_dict(torch.load(args.pretrained_path))
    ...
    train_dataset = IQADataset.KVQ_dataloader(database_dir,
                                              filename_list,
                                              transforms_train,
                                              database,
                                              crop_size,
                                              ...
                                              seed,
                                              spatial_sample_rate)
    test_dataset = IQADataset.KVQ_dataloader(val_database_dir,
                                              val_filename_list,
                                              transforms_test,
                                              database,
                                              crop_size,
    ...
    for epoch in range(num_epochs):
        ...
        for i, data_train in enumerate(train_loader):
            ...
            for param, ema_param in zip(model.parameters(), model_ema.parameters()):
                ema_param.data = ema_beta * ema_param.data + (1-ema_beta) * param.data
        ...
        with torch.no_grad():
            for i, data_test in enumerate(test_loader):
                ...
                outputs = model_ema(img_aesthetics, img_distortion)
            ...
            if test_SRCC + test_PLCC + test_KRCC - test_RMSE - test_MAE > best_test_criterion:
                ...
                save_model_name = os.path.join(args.snapshot, 
                                               args.model + '_' +  args.database + '_' + '_NR_' + str(args.spatial_sample_rate )+ '_' + str(args.crop_size) + '_' + 'epoch_%d_SRCC_%f_ema.pth' % (epoch + 1, test_SRCC))
                save_popt_name = os.path.join(args.snapshot, 
                                               args.model + '_' +  args.database + '_' + '_NR_' + str(args.spatial_sample_rate) + '_' + str(args.crop_size) + '_' + 'epoch_%d_SRCC_%f.npy' % (epoch + 1, test_SRCC))
                print("Update best model using best_val_criterion ")
                torch.save(model_ema.module.state_dict(), save_model_name)
